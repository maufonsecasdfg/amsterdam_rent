...
class YelpScraper():
    ...
    def get_businesses(self):
        ...
        for postcode in tqdm(self.coords['postcode']):
            lat = self.coords[self.coords['postcode']==postcode]['latitude'].values[0]
            lng = self.coords[self.coords['postcode']==postcode]['longitude'].values[0]

            for cat in categories:
                if cat not in self.cache[self.cache['postcode']==postcode]['category'].values:
                    tries = 0
                    while tries < 3:
                        query = f'{base_url}?categories={cat}&latitude={lat}&longitude={lng}&radius=2000&limit=50'
                    ...
                    self.cache = pd.concat([self.cache,cache])
    ...
    def save_new_bus(self):
        if (list(self.known_bus.columns) == ['id', 'name', 'url', 'review_count', 'rating', 'price', 'latitude','longitude', 'category'] 
            ...
            print('Either known_bus, new_bus or cache dataframe has incorrect schema.')
